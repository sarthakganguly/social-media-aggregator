...
async def connect_linkedin_account(
    ...
    async with httpx.AsyncClient() as client:
        ...
        if token_response.status_code != 200:
            raise HTTPException(status_code=400, detail="Could not get access token from LinkedIn")
        ...
        if profile_response.status_code != 200:
            raise HTTPException(status_code=400, detail="Could not fetch user profile from LinkedIn")
            
        profile_data = profile_response.json()
        linkedin_user_id = profile_data.get("sub")

    existing_account = db.query(SocialAccount).filter_by(
        provider="linkedin", provider_user_id=linkedin_user_id
    ).first()
    ...
    return {"status": "success", "provider": "linkedin", "username": profile_data.get("name")}
...
async def disconnect_account(
    ...
    account_to_delete = db.query(SocialAccount).filter_by(
        user_id=current_user.id, provider=request.provider
    ).first()

    if not account_to_delete:
        raise HTTPException(status_code=404, detail=f"{request.provider.capitalize()} account not found or already disconnected.")
